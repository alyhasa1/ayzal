...
export const action = async ({ request, context }: ActionFunctionArgs) => {
  const rateLimit = checkRequestRateLimit({
    request,
    namespace: "webhook:shipping:tcs",
    max: 180,
    windowMs: 60_000,
    scope: "path-and-ip",
  });
  ...
  try {
    ...
    const secret = env.SHIPPING_TCS_WEBHOOK_SECRET;
    const signature = request.headers.get("x-tcs-signature") ?? request.headers.get("x-signature");
    ...
    if (secret && !verification.valid) {
      console.warn("tcs webhook rejected", { reason: verification.reason });
      return json(
    ...
    console.info("tcs webhook processed", {
      shipment_id: body.shipment_id,
      status: String(body.status ?? "in_transit"),
