...
export default function AdminRoute() {
  ...
  return (
    <ClientConvexProvider fallback={<BrandLoader label="Loading admin workspace..." withNavOffset={false} />}>
      {isLoginRoute ? (
        <Outlet />
      ) : (
        <RequireAdmin>
...
export function ErrorBoundary() {
  ...
  return (
    <div className="min-h-screen bg-[#F6F2EE] text-[#111] px-4 py-14">
      <div className="max-w-xl mx-auto border border-[#111]/15 bg-white/90 p-6 sm:p-8">
        <p className="text-xs uppercase tracking-[0.3em] text-[#111]/55 mb-2">
          Admin Panel
        </p>
        <h1 className="font-display text-xl mb-3">Something went wrong</h1>
